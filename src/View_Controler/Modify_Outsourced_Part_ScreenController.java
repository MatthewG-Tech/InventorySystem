 ...
 */
public class Modify_Outsourced_Part_ScreenController implements Initializable {
    ...
    @FXML
    private TextField modifyPartCompanyField;
    @FXML
    ...
    private void inHouseButtonAction(ActionEvent event) throws IOException{
        ...
        modifyInhouseController.setUp(selectedPartIndex,selectedPart);
        stage.show();
        System.out.println("Insourced");
    }
    ...
    private void saveButtonAction(ActionEvent event) throws IOException{
        ...
        if(modifyPartNameField.getText().length() != 0 && modifyPartPriceField.getText().length() != 0 && modifyPartMinField.getText().length() != 0 && modifyPartMaxField.getText().length() != 0 && modifyPartCompanyField.getText().length() != 0 && validInput()){
            ...
            String companyName = modifyPartCompanyField.getText();
            if(min>max){
                ...
                Part temp = new Outsourced_Part( id, name,  price,  stock,  min,  max,  companyName);
    ...
    public void setUp(int index, Part part){
        ...
        if(part instanceof Outsourced_Part){
            Outsourced_Part temp = (Outsourced_Part) selectedPart;
            modifyPartCompanyField.setText(temp.getCompanyName()+"");
            
        }else{
            modifyPartCompanyField.setText("");
        }
    ...
    private boolean validInput(){
        if(isStringDouble(modifyPartPriceField.getText()) && isStringInt(modifyPartMinField.getText()) && isStringInt(modifyPartMaxField.getText()) && (modifyPartInventoryField.getText().length() != 0 || isStringInt(modifyPartInventoryField.getText()))){
            return true;
        }
        return false;
    }
